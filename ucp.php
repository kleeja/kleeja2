...
switch (g('go', 'str', ''))
{
	case 'login' :
		...
		$forget_pass_link	= !empty($forgetpass_script_path) && (int) $config['user_system'] != 1 ? $forgetpass_script_path :  get_url_of('get_pass');
		$register_link	= !empty($register_script_path) && (int) $config['user_system'] != 1 ? $register_script_path : get_url_of('register');
		...
		if (!$user->can('access_fileusers') && !$user_himself)
		{
		...
		if (!$user->can('access_fileuser') && $user_himself)
		{
		...
		#fileuser is closed ?
		if ((int) $config['enable_userfile'] != 1 && !$user->can('enter_acp'))
		{
		...
		if(!$data_user['show_my_filecp'] && ($user->data['id'] != $user_id) && !$user->can('enter_acp'))
		{
			kleeja_info($lang['USERFILE_CLOSED'], $lang['CLOSED_FEATURE']);
		}
