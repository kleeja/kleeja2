...
class user
{
	...
	public function login($name, $pass, $hashed = false, $expire = 86400, $loginadm = false)
	{
		...
		#if return was used by the hook
		if($return != 'augest3')
		{
			return $return;
		}
	...
	public function usernamebyid($user_id)
	{
		...
		($hook = $plugin->run_hook('usernamebyid_func_usr_class')) ? eval($hook) : null; //run hook
	...
	public function kleeja_check_user()
	{
		...
		#add other info to $user->can(data
		$this->extend_data();
	...
	/**
	 * Add other important datas to $user->can(data like ip
	 *
	 * @return void
	 */
	private function extend_data()
	{
		...
		#if a someone used the capital letters
	...
	public function is_bot($record = false)
	{
		...
		#update stats
		update_stats('last_' . $bot_name, $time, '');
		update_stats($bot_name . '_num');
	...
	/**
	 * Check if the current user can do a specfic ACL
	 ...
	 * @return bool true or false
	 */
	public function can($acl_name, $group_id = 0)
	{
		global $d_groups;
