...
class uploading
{
	...
	{
		global $SQL, $dbprefix, $config, $lang, $plugin, $user;

		($hook = $plugin->run_hook('process_func_uploading_cls')) ? eval($hook) : null; //run hook


		#To prevent flooding, user must wait, waiting-time is grapped from Kleeja settings, admin is exceptional
		if(!$user->can('enter_acp') && user_is_flooding())
		{
		...
		foreach($files as $file)
		{
			...
			($hook = $plugin->run_hook('uploading_process_func_loop_files')) ? eval($hook) : null; //run hook
			...
			if($upload_result  && !$just_check)
			{
				...
				$this->results[$insert_id]['name'] = $file['name'];
	...
	{
		global $SQL, $dbprefix, $config, $lang, $user, $plugin;
		...
		($hook = $plugin->run_hook('add_to_database_qr_uploading_cls')) ? eval($hook) : null; //run hook
		...
		# update Kleeja stats
		update_stats($is_img ? "total_images" : "total_files");
		update_stats('total_sizes', intval($size));

		($hook = $plugin->run_hook('add_to_database_end_uploading_cls')) ? eval($hook) : null; //run hook
